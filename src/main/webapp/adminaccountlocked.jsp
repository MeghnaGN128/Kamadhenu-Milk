...
<head>
    <title>Admin Account Locked - Kamadhenu Milk Products</title>
...
<!-- Navbar with light yellow color -->
...
<div class="container-box">
    ...
    <div class="alert">
        <c:choose>
            <c:when test="${not empty lockMessage}">
                ${lockMessage}
            </c:when>
            <c:otherwise>
                Your account has been locked after multiple unsuccessful login attempts.
            </c:otherwise>
        </c:choose>
    </div>

    <p>Enter your registered email to receive a secure link to set a new password.</p>
    ...
    <form action="${pageContext.request.contextPath}/sendResetLink" method="post">
        ...
        <p class="note">We will send a password reset link if an account exists for this email.</p>
    </form>
</div>
